...
columns = ['server_dttm', 'event_dttm', 'symbol_name', 'trade_id', 'price', 'quantity', 'direction']
...
class MyParser:
    ...
    def on_message_func_default(self):
        def foo(_wsa, raw_data):
            ...
            # Remove non used parts
            pre_data.pop('e', None)
            pre_data.pop('b', None)
            pre_data.pop('a', None)
            pre_data.pop('T', None)

            pre_data = list(pre_data.values())
            data = [round(datetime.now(timezone.utc).timestamp() * 1000), *pre_data]

            self.cache.append(data)

            if len(self.cache) >= 50 or datetime.now(timezone.utc).hour > self.hour_now or datetime.now(timezone.utc).day > self.day_now or self.montn_now < datetime.now(timezone.utc).month or datetime.now(timezone.utc).year > self.year_now:

                if datetime.now(timezone.utc).hour > self.hour_now or datetime.now(timezone.utc).day > self.day_now:
                    self.hour_now = datetime.now(timezone.utc).hour
                    self.day_now = datetime.now(timezone.utc).day
                    self.montn_now = datetime.now(timezone.utc).month
                    self.year_now = datetime.now(timezone.utc).year
...
def listen_stream(my_obj, url=None, on_open_func=None, on_message_func=None):
    if url is None:
        url = 'wss://nbstream.binance.com/eoptions/stream'
...
def _writer_maker():
    ...
    type_stream = stream_name.split('@')[1]
